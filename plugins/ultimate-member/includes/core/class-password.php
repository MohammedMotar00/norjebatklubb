...
if ( ! class_exists( 'um\core\Password' ) ) {
	...
	class Password {
		...
		function um_reset_password_errors_hook( $args ) {
			...
			if ( empty( $user ) ) {
				UM()->form()->add_error('username_b', __( 'Vänligen ange ditt användarnamn eller e-postadress', 'ultimate-member' ) );
			}
		...
		function um_change_password_errors_hook( $args ) {
			...
			if ( isset( $args['user_password'] ) && empty( $args['user_password'] ) ) {
				UM()->form()->add_error( 'user_password', __( 'Du måste ange ett nytt lösenord', 'ultimate-member' ) );
			}
			...
			if ( isset( $args['confirm_user_password'] ) && empty( $args['confirm_user_password'] ) ) {
				UM()->form()->add_error( 'confirm_user_password', __( 'Du måste bekräfta ditt nya lösenord', 'ultimate-member' ) );
			}

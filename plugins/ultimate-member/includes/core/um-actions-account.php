		...
		switch ( $_POST['_um_account_tab'] ) {
			case 'delete': {
				...
				if ( UM()->account()->current_password_is_required( 'delete' ) ) {
					if ( strlen( trim( $_POST['single_user_password'] ) ) == 0 ) {
						UM()->form()->add_error( 'single_user_password', __( 'Du måste ange ditt lösenord', 'ultimate-member' ) );
					} else {
						if ( ! wp_check_password( $_POST['single_user_password'], $user->data->user_pass, $user->data->ID ) ) {
							UM()->form()->add_error( 'single_user_password', __( 'Det här är inte ditt lösenord', 'ultimate-member' ) );
						}
					}
				}
					...
					if ( UM()->account()->current_password_is_required( 'password' ) ) {
						if ( empty( $_POST['current_user_password'] ) ) {
							...
							if ( ! wp_check_password( $_POST['current_user_password'], $user->data->user_pass, $user->data->ID ) ) {
								UM()->form()->add_error('current_user_password', __( 'Det här är inte ditt lösenord', 'ultimate-member' ) );
								return;
							}
						}
					}
			...
			case 'general': {
				...
				if ( isset( $_POST['first_name'] ) && ( strlen( trim( $_POST['first_name'] ) ) == 0 && $account_name_require ) ) {
					UM()->form()->add_error( 'first_name', __( 'Du måste ange ditt förnamn', 'ultimate-member' ) );
				}

				if ( isset( $_POST['last_name'] ) && ( strlen( trim( $_POST['last_name'] ) ) == 0 && $account_name_require ) ) {
					UM()->form()->add_error( 'last_name', __( 'Du måste ange ditt efternamn', 'ultimate-member' ) );
				}

				if ( isset( $_POST['user_email'] ) ) {

					if ( strlen( trim( $_POST['user_email'] ) ) == 0 ) {
						UM()->form()->add_error( 'user_email', __( 'Du måste ange din e-post', 'ultimate-member' ) );
					}
...
function um_after_account_privacy( $args ) {
	...
	<div class="um-field um-field-export_data">
		<div class="um-field-label">
			<label>
				<?php esc_html_e( 'Ladda ner dina data', 'ultimate-member' ); ?>
			</label>
			<span class="um-tip um-tip-<?php echo is_rtl() ? 'e' : 'w' ?>" original-title="<?php esc_attr_e( 'Du kan begära en fil med den information som vi anser är mest relevant och användbar för dig.', 'ultimate-member' ); ?>">
		...
		if ( ! empty( $pending ) && $pending['post_status'] == 'request-pending' ) {
			...
			if ( UM()->account()->current_password_is_required( 'privacy_download_data' ) ) { ?>
				...
				<label name="um-export-data">
					<?php esc_html_e( 'Ange ditt nuvarande lösenord för att bekräfta en ny export av dina personuppgifter.', 'ultimate-member' ); ?>
				</label>
				<div class="um-field-area">
					<input id="um-export-data" type="password" placeholder="<?php esc_attr_e( 'Lösenord', 'ultimate-member' )?>">
					<div class="um-field-error um-export-data">
						<span class="um-field-arrow"><i class="um-faicon-caret-up"></i></span><?php esc_html_e( 'Du måste ange ett lösenord', 'ultimate-member' ); ?>
					</div>
			...
			<a class="um-request-button um-export-data-button" data-action="um-export-data" href="javascript:void(0);">
				<?php esc_html_e( 'Begär data', 'ultimate-member' ); ?>
			</a>
		<?php } ?>
	...
	<div class="um-field um-field-export_data">
		<div class="um-field-label">
			<label>
				<?php esc_html_e( 'Radera dina data', 'ultimate-member' ); ?>
			</label>
			<span class="um-tip um-tip-<?php echo is_rtl() ? 'e' : 'w' ?>" original-title="<?php esc_attr_e( 'Du kan begära radering av de uppgifter som vi har om dig.', 'ultimate-member' ); ?>">
				<i class="um-icon-help-circled"></i>
			</span>
			<div class="um-clear"></div>
		</div>
		...
		if ( ! empty( $pending ) && $pending['post_status'] == 'request-pending' ) {
			...
			if ( UM()->account()->current_password_is_required( 'privacy_erase_data' ) ) { ?>
				...
				<label name="um-erase-data">
					<?php esc_html_e( 'Ange ditt nuvarande lösenord för att bekräfta raderingen av dina personuppgifter.', 'ultimate-member' ); ?>
					<input id="um-erase-data" type="password" placeholder="<?php esc_attr_e( 'Lösenord', 'ultimate-member' )?>">
			...
			<a class="um-request-button um-erase-data-button" data-action="um-erase-data" href="javascript:void(0);">
				<?php esc_html_e( 'Begär data raderas', 'ultimate-member' ); ?>
			</a>
		<?php } ?>
	...
	if ( $_POST['request_action'] == 'um-export-data' ) {
		if ( UM()->account()->current_password_is_required( 'privacy_download_data' ) ) {
			if ( ! wp_check_password( $password, $hash ) ) {
				$answer = esc_html__( 'Lösenordet du angav är felaktigt.', 'ultimate-member' );

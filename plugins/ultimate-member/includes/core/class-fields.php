			...
			switch ( $array['type'] ) {
				...
				case 'image':
					...
					if ( ! isset( $array['button_text'] ) ) {
						$array['button_text'] = __( 'Ladda upp', 'ultimate-member' );
					}
				...
				case 'password':
					...
					if ( $key == 'single_user_password' ) {
						...
						if ( UM()->account()->current_password_is_required( 'password' ) &&
							...
							if ( isset( $data['label'] ) ) {
								$output .= $this->field_label( __( 'Nuvarande lösenord', 'ultimate-member' ), $key, $data );
							}
						...
						if ( $this->set_mode == 'account' && um_is_core_page( 'account' ) || $this->set_mode == 'password' && um_is_core_page( 'password-reset' ) ) {
							...
							$output .= $this->field_label( __( 'Nytt lösenord', 'ultimate-member' ), $key, $data );
				...
				case 'image':
					...
					$output .= '<div class="um-modal-footer">
									<div class="um-modal-right">
										<a href="javascript:void(0);" class="um-modal-btn um-finish-upload image disabled" data-key="' . $key . '" data-change="' . __( 'Change photo', 'ultimate-member' ) . '" data-processing="' . __( 'Processing...', 'ultimate-member' ) . '"> ' . __( 'Tillämpa', 'ultimate-member' ) . '</a>
										<a href="javascript:void(0);" class="um-modal-btn alt" data-action="um_remove_modal"> ' . __( 'Avbryt', 'ultimate-member' ) . '</a>
									</div>
...
//						$atts_ajax .= ' data-um-ajax-url="' . esc_url( $ajax_source_url ) . '" ';
		...
		function display_view( $mode, $args ) {
			...
			if ( UM()->options()->get( 'profile_empty_text' ) ) {
				...
				if (um_is_myprofile()) {
					$output .= '<p class="um-profile-note">' . $emo . '<span>' . sprintf( __( 'Din profil ser lite tom ut. Varför inte <a href="%s">lägga till</a> lite information!', 'ultimate-member' ), um_edit_profile_url() ) . '</span></p>';
				} else {

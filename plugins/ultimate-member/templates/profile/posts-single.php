...
<div class="um-item">
	...
	<div class="um-item-meta">
		...
		<span>
			<?php _e( 'i', 'ultimate-member' ); ?>: <?php the_category( ', ', '', $post->ID ); ?>
		</span>
		<span>
			<?php $num_comments = get_comments_number( $post->ID );

			if ( $num_comments == 0 ) {
				$comments = __( 'inga kommentarer', 'ultimate-member' );
			} elseif ( $num_comments > 1 ) {
				$comments = sprintf( __( '%s kommentarer', 'ultimate-member' ), $num_comments );
			} else {
				$comments = __( '1 kommentar', 'ultimate-member' );
			} ?>

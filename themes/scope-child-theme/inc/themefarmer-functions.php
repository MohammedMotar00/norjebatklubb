...
<ul class="header-topbar-links">
  ...
  <li><a href="<?php echo esc_url($social['link']); ?>" <?php echo absint($new_tab) ? 'target="_blank"' : ''; ?>><i
...
{
  $top_phone = get_theme_mod('scope_top_phone');
  if (!empty($top_phone)) : ?>
<span class="contact-item contact-mobile"><i class="fa fa-phone"></i><span class="contact-link"><a
      href="callto:<?php echo esc_attr($top_phone); ?>"><?php echo esc_html($top_phone); ?></a></span></span>
<?php endif; ?>
<?php $top_email = get_theme_mod('scope_top_email');
  if (!empty($top_email)) : ?>
<span class="contact-item contact-email"><i class="fa fa-envelope"></i><span class="contact-link"><a
      href="mailto:<?php echo esc_attr($top_email); ?>"><?php echo esc_html($top_email); ?></a></span></span>
<?php endif;
}
...
function scope_get_page_links_html()
{
  if (scope_is_wc()) {
    ...
    foreach ($links as $key => $link) {
      $html .= sprintf(
        '<li><a class="top-bl-%1$s" href="%2$s"> %3$s </a></li>',
        esc_attr($key),
        esc_url($link),
...
<div class="row the-comment">
  ...
  <div class="col-10">
    <div class="comment-items">
      ...
      <em
        class="comment-item comment-awaiting-moderation"><?php esc_html_e('Your comment is awaiting moderation.', 'scope'); ?></em>
      <?php endif; ?>
    </div>
...
function scope_get_standard_fonts()
{
  return apply_filters(
    'themefarmer_standard_fonts_array',
    array(
...
{
  return apply_filters(
    'themefarmer_google_fonts_array',
...
function scope_home_sections_init()
{
  ...
  $default_sections =  apply_filters('scope_home_page_default_sections', array('slider', 'services', 'products-latest', 'team', 'callout', 'testimonials', 'brands',  'subscribe', 'blog', 'about', 'contact'));
    ...
    if (get_theme_mod('scope_is_enable_section_' . $section_name, true)) {
      if (function_exists('scope_homepage_section_' . $section_name)) {
        if ($section_name == "about") {
          add_action('scope_print_home_page_sections', 'scope_homepage_section_' . $section_name, 1);
        } else {
